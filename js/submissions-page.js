...
function darkMode() {
  ...
  dark.href = "../css/submissions-page-darkmode.css";
}

function lightMode() {
  const light = document.querySelector("link");
  light.href = "../css/submissions-page.css";
}

//Add new image from Submit Button

const submitBtn = document.querySelector("#submitBtn");
submitBtn.addEventListener("click", (e) => {
  e.preventDefault(); 
  //a button on a form normally refreshes a page. We use preventDefault to avoid that
  //next get values from the form
  const name = document.querySelector('input[name="name"]').value;
  const title = document.querySelector('input[name="title"]').value;
  const imgUrlOne = document.querySelector('input[name="imgUrlOne"]').value;
  const imgUrlTwo = document.querySelector('input[name="imgUrlTwo"]').value;
  const imgUrlThree = document.querySelector('input[name="imgUrlThree"]').value;
  const imgUrlFour = document.querySelector('input[name="imgUrlFour"]').value;
  const imgUrlFive = document.querySelector('input[name="imgUrlFive"]').value;

  const newImageDetail = document.createElement("div");
  newImageDetail.classList.add("cover-pic");
  newImageDetail.innerHTML = 
    ...
    //NOTE: MUST USE BACKTICKS FOR THIS TO WORK
    //NOTE: ONLY ONE IMAGE WILL BE CREATED TO REPRESENT THE COVER PICTURE AS INSTRUCTED WHEN SUBMITTING THE FORM

  const coverPicsContainer = document.querySelector(".cover-pics-container");
  coverPicsContainer.appendChild(newImageDetail);
